  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-semibold text-stone-800 mb-2">Bạn có dị ứng thực phẩm nào?</h2>
        ...
        {commonAllergens.map((allergen) => (
          <button
            ...
            className={`p-4 rounded-lg border-2 transition-all ${
              allergens.includes(allergen.id) ? 'border-orange-500 bg-orange-50' : 'border-stone-200 hover:border-stone-300 bg-white'
            }`}
          >
            <div className="text-center">
              <div className="text-3xl mb-2">{allergen.emoji}</div>
              <div className="text-sm font-medium text-stone-800">{allergen.name}</div>
              {allergens.includes(allergen.id) && <Check className="w-4 h-4 text-orange-500 mx-auto mt-2" />}
            </div>
          </button>
        ))}
      </div>
      ...
      <div className="pt-6">
        <div className="flex gap-2">
          <Input
            ...
            className="flex-1"
          />
          <button onClick={addCustomAllergen} className="px-6 py-2 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-colors">
            Thêm
          </button>
        </div>
        ...
        {customAllergens.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {customAllergens.map((item) => (
              <Badge key={item} variant="secondary" className="px-3 py-1 cursor-pointer hover:bg-stone-300" onClick={() => removeCustomAllergen(item)}>
                {item} ✕
              </Badge>
            ))}
          </div>
